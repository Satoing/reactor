...
TcpServer::TcpServer(const std::string &ip, uint16_t port, int threadnum): threadnum_(threadnum) {
    ...
    pool_ = new ThreadPool(threadnum_, "IO");
    for(int i = 0; i < threadnum_; i++) {
        ...
        pool_->addtask(std::bind(&EventLoop::run, subloops_[i]));
    }
}
...
TcpServer::~TcpServer() {
    ...
    for(EventLoop *subloop : subloops_) delete subloop;
    delete pool_;
}
...
void TcpServer::newconnection(Socket *connsock) {
    ...
    Connection *connection = new Connection(subloops_[connsock->fd()%threadnum_], connsock);
